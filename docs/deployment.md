...
Icon files live in `landing/public/` and are referenced by Godot's export preset:

| File | Size | Use |
|------|------|-----|
| `index.png` | 512×512 | Android splash screen, high-res devices |
| `index.icon.png` | 144×144 | Android home screen, app drawer |
| `index.apple-touch-icon.png` | 180×180 | iOS home screen |

To regenerate: replace these files and re-export from Godot. They are not regenerated by `build.sh`.
...
## PWA Testing (Add to Home Screen)

After deploying, test the PWA on a mobile device:

1. Open the game URL on your mobile browser
2. Wait for the game to fully load (dictionary pre-fetch completes)
3. On **iOS Safari**: tap Share → "Add to Home Screen"
   On **Android Chrome**: you should see an install banner, or tap the menu → "Install app"
4. The app installs with the Word Loom icon and standalone (no browser chrome) experience
5. **Test offline**: enable airplane mode, then open the app from home screen — gameplay should work fully

If the install prompt doesn't appear:
- Verify `dist/.well-known/manifest.json` exists after export
- Clear browser cache and try again
- Chrome on Android requires service worker + manifest before showing the prompt
