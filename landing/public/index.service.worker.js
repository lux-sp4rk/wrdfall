...
/** @type {string} */
const CACHE_VERSION = '1776997548|2817095';
/** @type {string} */
...
function ensureCrossOriginIsolationHeaders(response) {
	// Network errors have status 0; Response constructor rejects status outside [200, 599]
...
async function fetchAndCache(event, cache, isCacheable) {
	...
	if (response == null) {
		...
		try {
			...
			console.error('Fetch failed: ', e); // eslint-disable-line no-console
			// Return a network error response so ensureCrossOriginIsolationHeaders doesn't crash
		...
		if (isNavigate || isCachable) {
			...
			event.respondWith((async () => {
				try {
					...
					console.error('Fetch failed (cross-origin headers): ', e); // eslint-disable-line no-console
					return new Response('Network error', { status: 503, statusText: 'Service Unavailable' });
				}
			})());
		}
