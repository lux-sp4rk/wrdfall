...
**Note on Architecture:** This project serves the web build from the `dist/` directory. `dist/` is generated by CI (GitHub Actions) via `npm run build:all` — it is never committed directly.
...
| Doc | Contents |
|---|---|
| [`AGENTS.md`](AGENTS.md) | Agent-assisted project (Ferris bot handles bug reports and fixes) |
